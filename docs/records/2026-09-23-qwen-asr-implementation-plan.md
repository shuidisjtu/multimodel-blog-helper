...
> 状态：**A6 全部完成**（A6-1 ~ A6-7，2026-09-25），转录能力已恢复并交付句级时间戳  
...
**已落地**：在业务空间专属域名上真实跑通两个样本（15.0 s 中文 / 170.9 s 英文），全链路 `succeeded`；时间戳程序化校验通过（31 段、无重叠、单调、覆盖 0→170.08 s），与 A6-1 通用域名的实测逐项吻合；两条上限分别用同一样本隔离验证（`413 FILE_TOO_LARGE` / `400 AUDIO_TOO_LONG`）；指标落盘字段非空；日志脱敏自检命中 0。证据见 [A6-7 证据](../evidence/a6-7-integration/2026-09-25-a6-7-integration-shuidisjtu.md)。
