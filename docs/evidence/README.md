...
> 本地预览记录于基线 `fe699f5`（C4 改动随后提交为 `d24964e` 并与 `main` 合并）；此处是**未提交工作区**的本机验收，正式 `release-<sha>` 和 Actions 成功记录待在 `main` 触发流水线后归档。所列「防篡改」结论限于未重算清单的篡改——`release:check` 的自洽校验没有外部锚点，与可信清单逐项比对须用 `--expect`。
