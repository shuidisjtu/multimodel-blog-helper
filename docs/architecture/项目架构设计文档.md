...
将已完成的音频转录、文本摘要、天气工具调用和 Express 示例整合为一个可维护、可测试、可观测的服务：用户上传音频后获得任务标识；服务完成转录与摘要并保存产物；用户可查询任务与下载文本；天气能力作为独立外部工具供助手调用。系统同时提供健康检查、统一错误处理、最小可行 CI/CD 和关键过程记录。
...
| OpenAPI、单元/集成/E2E 测试、GitHub Actions、部署健康检查 | 本地 Whisper 模型（约 2GB，作为独立可选实现） |
...
上传默认限制：仅 `audio/mpeg`、`audio/wav`、`audio/mp4`、`audio/x-m4a`；最大 25 MB（可配置，但不得超过转录提供方限制）；文件名仅作展示，存储名由服务生成；拒绝路径分隔符和 MIME/魔数不一致的文件。通过 `Idempotency-Key` 支持 24 小时内重复提交返回同一 Job。

## 6. 关键处理流程

1. HTTP 层生成/接收 `requestId`，校验 multipart 字段、大小、MIME 和文件签名；将输入以随机 `jobId` 写入临时区。
...
| 端口 | 最小接口 | 实现 |
| --- | --- | --- |
| `Transcriber` | `transcribe(file): Transcript` | `OpenAITranscriber`；后续 `LocalWhisperTranscriber` |
...
| `OPENAI_TRANSCRIBE_MODEL`、`OPENAI_SUMMARY_MODEL` | 是 | 明确记录实际模型版本 |
...
- 设置请求体、上传、上游 HTTP 的独立超时；请求中断时停止后续队列投递并清理未关联文件。
...
每两周进行 30 分钟架构健康检查：删除已过期 feature flag/字段，审查 90 天无人改动但稳定的模块是否可版本化封装，核对运行日志中的高频错误，核对 ADR 与实现是否仍一致。每次检查产出一条 `docs/records/YYYY-MM-DD-architecture-health.md`，记录结论、证据、责任人与截止日，作为答辩材料的一部分。
...
| P3：质量与交付（2 天） | 测试金字塔、CI、部署、E2E、运行手册 | PR 门禁全绿；staging smoke test；失败可定位 |
