...
| Qiu Wenxi | DevOps 与质量负责人 | GitHub Actions、构建制品、部署流程、健康监测、运行告警、质量门禁、发布与回滚记录 | 错误处理、E2E、项目总结 | 模型提示词和天气数据语义 |
...
| C2 | 构建制品与部署流水线 | 制品带 commit SHA；部署 staging 后执行 readiness 与 smoke test |
| C3 | 健康监测与告警 | `/health/live` 和 `/health/ready` 语义不同；连续失败有可验证通知 |
| C4 | E2E 与发布检查单 | fixture 音频能完成最短闭环；上线前检查单有执行人和时间 |
| C5 | 回滚与故障运行手册 | 包含失败判定、日志定位、回滚步骤、恢复验证 |
...
| 部署与 E2E 报告 | Qiu | Zhang | staging URL、版本 SHA、测试结果、回滚版本 | 技术负责人完成上线批准或退回 |
...
- 各 DRI 更新：昨日完成、今日计划、阻塞项、是否影响依赖。
- 阻塞超过 4 小时即创建 Issue；涉及密钥、费用、外部服务不可用时立即通知全员。
- 合并前最少由代码所有权表指定的一位审核人完成评审。

### 每周

- 周初：确认本周工作包、依赖和可验收演示场景。
- 周中：做一次完整联调（上传→转录→摘要→查询/下载；天气→错误降级）。
- 周末：Qiu 发布 CI/测试/部署摘要；Zhang 主持 30 分钟架构健康检查；Luan 更新 API 变更记录。
...
| P1 | 核心音频流程大面积失败 | 当日定位并修复或回滚 | Zhang / Luan |
| P2 | 单一接口异常、可绕过问题 | 下一个工作日排期 | 对应模块 DRI |
