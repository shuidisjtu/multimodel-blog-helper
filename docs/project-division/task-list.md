...
| B5 | 接口 DTO 与契约测试 | B1、B2、B4 | 对现有 `/api/v1` 路由校验请求/响应 DTO、状态码、错误 envelope 与媒体类型；OpenAPI 与实际响应一致 | 🔶 OpenAPI 与部分手写 DTO 校验已完成；契约测试待补齐 / ym-hello |
...
| C2 | 测试与覆盖率 CI（含契约测试） | C1、B5 | 保持覆盖率阈值 ≥80%；契约测试进入 CI；当前 237 项测试与覆盖率基线持续通过 | 🔶 测试/覆盖率 CI 已完成；契约测试接入待完成 / ym-hello |
