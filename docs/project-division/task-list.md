...
> 版本：v1.6 ｜ 用途：展示项目需要完成的工作与验收标准 ｜ 更新：2026-08-30
>
...
| --- | --- | --- | --- | --- |
| D1 | 展示目标与叙事设计 | A5、B1、B2、B4 | 明确老师需要理解的产品场景、核心闭环、技术讲解主线、当前边界和后续方向；确定 PPT 结构与必展示证据 | 待办/P0 |
| D2 | 最小 Web 工作台 | B1、B2、B4、D1 | 独立 `web/` 前端真实调用现有 API；支持音频上传、Job 状态轮询、摘要/转录展示、转录下载和天气查询；关键交互有测试或 mock 验证 | 待办/P0 |
| D3 | PPT 汇报材料与证据整合 | D1、D2 | PPT 作为唯一正式展示物，嵌入 Web 画面、真实运行录屏/截图、架构图、状态机、问题解决、稳定性数据和后续方向；无项目运行环境时仍可完成展示 | 待办/P0 |

> D2 的现场实时启动和离线 replay 均不是硬性验收条件。离线 replay 仅作为可选备用；PPT 中必须明确区分真实运行结果与离线展示。
...
| 待分配 | D1 → D2 → D3 | 答辩叙事、最小 Web 工作台与 PPT 证据整合 |
