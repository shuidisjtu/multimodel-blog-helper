...
将第 3、4 章示例整合为可部署的学习研究型 HTTP 服务：上传音频 → 异步转录 + 摘要 → 查询/下载；另含天气工具调用、健康监测与 CI。方案与计划已冻结，按任务清单驱动。当前进度：**A1–A5、B3 与 B5 已完成**(架构 ADR / OpenAI 适配器 / 任务用例 / 重试超时策略 / 答辩材料 / 上传校验与临时文件策略 / OpenAPI 接口契约,158 测试,覆盖率全维度 ≥80%)；下一步 B1(上传受理接口)。
...
> **当前阶段说明**：重构主线仍在开发中（A1–A5、B3 与 B5 已完成，OpenAPI 契约已落地；HTTP 服务启动入口和 B1 上传受理仍待实现）。上面的快速开始验证的是**代码质量与测试环境**（依赖/类型/Lint/文档/结构/测试全绿）；当前 `npm run dev` 仍因 HTTP 服务启动入口尚未落地而无法启动，服务功能演示（上传音频 → 异步转录/摘要 → 查询下载）将在 B1/B2 等 HTTP 任务完成后补充。可直接运行的教材示例见下方“快速开始(教材示例)” 。
