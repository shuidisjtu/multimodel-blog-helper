...
将第 3、4 章示例整合为可部署的学习研究型 HTTP 服务：上传音频 → 异步转录 + 摘要 → 查询/下载；另含天气工具调用、健康监测与 CI。方案与计划已冻结，按任务清单驱动。当前进度：**A1–A5 已完成**(架构 ADR / OpenAI 适配器 / 任务用例 / 重试超时策略 / 答辩材料,126 测试,覆盖率全维度 ≥80%)；下一步 B 系列(HTTP 接口层)。
...
> **当前阶段说明**：重构主线仍在开发中（A1–A5 完成，HTTP 接口层 B 系列未开始）。上面的快速开始验证的是**环境就绪**（依赖/配置/测试全绿）；服务功能演示（上传音频 → 异步转录/摘要 → 查询下载）将在 B 系列完成后补充。可直接运行的教材示例见下方"快速开始(教材示例)"。
