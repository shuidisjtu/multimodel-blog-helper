/**
 * 上传校验规则: 白名单 MIME + 大小 + 魔数一致性。
 * 纯函数、无 IO; 内存快速校验在 HTTP 层调用, 时长探测在用例层落盘后(见 AudioDurationProbe)。
 */

export type AudioCodecFamily = 'mp3' | 'wav' | 'mp4';

/** 白名单 MIME(相比教材 03-02 的 audio/* 前缀匹配, 有意收紧防伪造 MIME)。 */
...
/** MIME → 扩展名(存储名由服务生成); 与魔数家族解耦: m4a 与 mp4 同容器, 扩展名仍保留原始类型。 */
...
/**
 * 魔数探测(实现注意): mp3 无固定文件头 —— ID3 帧头或 MPEG 同步字(宽松匹配, 不得误杀合法 mp3);
